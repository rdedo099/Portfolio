...
public class signInServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		String query = "SELECT COUNT(*) FROM Project_Hotel.customers WHERE ssn ='"+ id+"'";
		...
		try {
			...
			if (resId != 0) {
			String html = "<html> <button onclick = \"parent.location='CustHome.jsp'\">Go</button> </html>";
			writer.println(html);
			} else {
			String html = "<html> <button onclick = \"parent.location='CustLogin.jsp'\">Incorrect SSN, try again</button></html>";
			writer.println(html);	
			}
			result.close();
			state.close();
